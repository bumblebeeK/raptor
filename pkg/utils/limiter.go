...
var log = base.NewLogWithField("sub-sys", "rate")
...
func (l *APILimiter) requestFinished(r *limitedRequest, err error) {
	...
	if l.params.Log {
		log.Infof("API call has been processed")
	} else {
		log.Debugf("API call has been processed")
	}
...
func (l *APILimiter) wait(ctx context.Context) (req *limitedRequest, err error) {
	...
	case <-ctx.Done():
		if l.params.Log {
			log.Warnf("Not processing API request due to cancelled context")
		}
		l.mutex.Unlock()
	...
	if l.params.Log {
		log.Infof("Processing API request with rate limiter")
	} else {
		log.Debugf("Processing API request with rate limiter")
	}
	...
	if parallelRequests > 0 {
		...
		if err2 != nil {
			if l.params.Log {
				log.Warnf("Not processing API request. Wait duration for maximum parallel requests exceeds maximum")
			}
	...
	if (l.params.MaxWaitDuration > 0 && (limitWaitDuration+req.waitDuration) > l.params.MaxWaitDuration) || limitWaitDuration == rate.InfDuration {
		if l.params.Log {
			log.Warnf("Not processing API request. Wait duration exceeds maximum")
		}
	...
	if limitWaitDuration != 0 {
		...
		case <-ctx.Done():
			if l.params.Log {
				log.Warnf("Not processing API request due to cancelled context while waiting")
			}
...
skipRateLimiter:
	...
	if l.params.Log {
		log.Infof("API request released by rate limiter")
	} else {
		log.Debugf("API request released by rate limiter")
	}
